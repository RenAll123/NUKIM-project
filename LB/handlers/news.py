...
from pexels_image import get_image_for_news
...
def to_bubble(item: Dict, idx: int = 0) -> BubbleContainer:
    hero_url = get_image_for_news(item.get("title", ""), item.get("snippet", "")) \
               or DEFAULT_IMAGES[idx % len(DEFAULT_IMAGES)]
    ...
    return BubbleContainer(
        hero=ImageComponent(url=hero_url, size="full", aspectMode="cover"),
